...
#
# Melissa Monk, NMFS
# Modifed by Chantel Wetzel
# February 3, 2017
# =============================================================================

# Read in funciton for formatting
...
# Split catch by regions -retaning the colunns for each -you'll have to edit
 ...
 #Exec_region1_catch = Exec_catch[,1]
...
#Exec_region1_catch = Exec_catch[,1:3]
...
Plot_catch = function(Catch_df) {
             ...
             scale_x_continuous(breaks=seq(Dat_start_mod1, Dat_end_mod1, 10)) +
             ylab('Landings (mt)')
}
...
Exec_catch_summary = read.csv('./txt_files/a_catches_ExecutiveSummary.csv')
...
align(Exec_catch.table) = c('l', # Leading ghost column for row.names
                            'l', # Year alignment
                            rep('>{\\centering}p{1in}', dim(Exec_catch.table)[2]-1)) # Fleets and total dead #, 
                            #'>{\\centering}p{1in}',
                            #'>{\\centering}p{1in}', 
                            #'>{\\centering}p{.9in}',
                            #'>{\\centering}p{.9in}',
...
for (model in 1:n_models) {
  ...
  SpawningByrs$Value = round(SpawningByrs$Value, sb.dig)
  ...
  Depletion$Value = round(Depletion$Value, digits=dep.dig)
  
  Depletionyrs$lowerCI = round(Depletionyrs$Value + 
                               qnorm(0.025)*Depletionyrs$StdDev, digits=dep.dig)
  ...
  Depletionyrs$CI = paste0(comma(Depletionyrs$lowerCI,dep.dig), '\u2013', comma(Depletionyrs$upperCI,dep.dig))
...
for (model in 1:n_models) {
  ...
  Recruityrs$CI = paste0(comma(Recruityrs$lowerCI, digits = sb.dig), '\u2013', comma(Recruityrs$upperCI, digits = sb.dig))
  
  Recruityrs$Value = comma(Recruityrs$Value, digits = sb.dig)
...
for (model in 1:n_models) {
  ...
  Exploityrs$lowerCI = comma(Exploityrs$Value +qnorm(0.025) * Exploityrs$StdDev, digits = dep.dig)
  
  Exploityrs$upperCI = comma(Exploityrs$Value - qnorm(0.025) * Exploityrs$StdDev, digits = dep.dig)
  
  Exploityrs$CI = paste0(Exploityrs$lowerCI, '\u2013', Exploityrs$upperCI)
  
  Exploityrs$Value = as.numeric(comma(Exploityrs$Value, digits = dep.dig))
  ...
  SPRratioyrs$lowerCI = comma(SPRratioyrs$Value +
                              qnorm(0.025) * SPRratioyrs$StdDev, digits = dep.dig)
  
  SPRratioyrs$upperCI = comma(SPRratioyrs$Value -
                              qnorm(0.025) * SPRratioyrs$StdDev, digits = dep.dig)
  
  SPRratioyrs$CI = paste0(SPRratioyrs$lowerCI, '\u2013', SPRratioyrs$upperCI)
  
  SPRratioyrs$Value = comma(SPRratioyrs$Value, digits = dep.dig)
...
for (model in 1:n_models) {
 if (model == 1){
  ...
  Ref_pts$lowerCI1 = ifelse(Ref_pts$lowerCI >= 1, as.character(round(Ref_pts$lowerCI, 0)), 
                            as.character(comma(Ref_pts$lowerCI, 3))) 
  
  Ref_pts$upperCI1 = ifelse(Ref_pts$upperCI>=1, as.character(round(Ref_pts$upperCI,0)), 
                            as.character(comma(Ref_pts$upperCI, 3))) 
  
  Ref_pts$CI1      = paste0(Ref_pts$lowerCI1, '\u2013', Ref_pts$upperCI1)
...
mngmnt = read.csv('./txt_files/f_Manage_ExecutiveSummary.csv')
...
# Extract OFLs for next 10 years for each model
      ...
      OFL_mod1 = comma(OFL_mod1[, 2], digits = sb.dig)    #OFL_mod1[c(-1,-2),2]
      
      ACL_mod1 = mod1$derived_quants[grep('ForeCatch_',mod1$derived_quants$LABEL),]
      ACL_mod1 = comma(ACL_mod1[,2], digits = sb.dig)
      
      #Turn into a dataframe and get the total
      Year = seq(Project_firstyr,Project_lastyr, 1)
      OFL = data.frame(Year, OFL_mod1, ACL_mod1)
...
# Model 1
  ...
  SPRratio_Exploit_mod1 = SPRratio_Exploit_mod1[2:dim(SPRratio_Exploit_mod1)[1],c(2,3)]
  ...
  Age5biomassyrs_mod1 = comma(Age5biomassyrs_mod1, digits = sb.dig)
  
  # Spawning biomass and depltion
  SpawnDeplete_mod1 = SpawnDeplete_mod1[,c(2:5)]
  SpawnDeplete_mod1[,1] = SpawnDeplete_mod1[,1]
  SpawnDeplete_mod1[,3] = SpawnDeplete_mod1[,3]
  #SpawnDeplete_mod1[,1] = SpawnDeplete_mod1[,2:3] #spawning biomass
  #SpawnDeplete_mod1[,3] = SpawnDeplete_mod1[,4:5] #depletion
  
  # Recruitment
  #Recruittab_mod1 = Recruittab_mod1[,c(2,3)]
  #Recruittab_mod1[,2] = Recruittab_mod1[,2]
  #Recruittab_mod1 = Recruittab_mod1[,c(1,2)]
  Recruittab_mod1 = cbind(Recruittab_mod1[,2],Recruittab_mod1[,3])
...
if (n_models == 1) {
  ...
  mngmnt.cut = subset(mngmnt,mngmnt$Year >=(FirstYR) & mngmnt$Year <=(LastYR))
  mngmnt.cut = mngmnt.cut[,c(1:2,4:dim(mngmnt)[2])] # remove the ABC
  base_summary = cbind(mngmnt.cut,mod1_summary)
  
  
  # Transpose the dataframe to create the table and create data labels  
  base_summary = as.data.frame(base_summary)
  base_summary = t(base_summary)
  rownames(base_summary)=c('Years',
                       'OFL (mt)',
                       'ACL (mt)',
                       'Landings (mt)', 
                       'Total Dead (mt)',
