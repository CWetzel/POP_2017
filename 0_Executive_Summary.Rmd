...
Due to the sensitivity associated with the assessment given the assumed steepness value the assessment is classified as a Category 2 stock assessment, with a default sigma of 0.72.  This default sigma is used to determine the catch reduction to account for scientific uncertainty because the estimated sigma for current spawning biomass in the assessment is (`r round(mod1$Pstar_sigma,2)`).
...
There are many areas of research that could be undertaked to benefit the understanding and assessment of `r spp`. Below, are issues that are considered of importance.
